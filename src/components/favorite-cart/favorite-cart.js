            ...
            <div className="favorite-card">
                ...
                <div className="description-favorite-cart-item">
                    <Link to={`/description/${id}/`} style={{ textDecoration: 'none' }}>
                        <div className="title-favorite-item">{title}</div>
                    </Link>
                    <div className="favorite-rate-and-price">
                        <div className="headphone-favorite-rate">
                            <div className="rate-favorite-item-vector">★</div>
                            <div className="rate-favorite-item">{rate}</div>
                        </div>
                        <div className="favorite-price-and-discount">
                            <div className="favorite-container-price">
                                <div
                                    className={`${
                                        discount_new
                                            ? 'price-favorite-with-discount'
                                            : 'favorite-price'
                                    }`}
                                >
                                    {price}₽
                                ...
                                {discount_new && (
                                    <div className="favorite-discount">{discount_new}</div>
                                )}
                            </div>
                            <div className="favorite-old-price">{oldprice_new}</div>
                        </div>
                    </div>
                ...
                <div className="button-delete">
                    <img
                        src={images['Button_Delete.png']}
                        alt={'Button_Delete'}
                        width="20px"
                        height="17px"
                        onClick={() => onDeleteFromFavorite(id)}
                    />
                </div>
            </div>
        );
            ...
            <div className="favorite-cart-all">
                <div className="thing-in-favorite">
                    {items.length > 0 ? (
                        <div>
                            {items.map((item) => {
                                return <div key={item.id}>{renderFavoriteCard(item)}</div>;
                            })}
                        </div>
                    ) : (
